...
const idTypeOptions = [
  ...
  const [selectedFromDate, setSelectedFromDate] = useState(editBookingData.startDate);
    ...
    onSuccess: (data) => {
      ...
      if (result && !result.available) {
        ...
        if (unavailableRoomNumbers && unavailableRoomNumbers.length > 0) {
          ...
          const errorMessage = `Room(s) ${unavailableRoomNumbers.join(", ")} not available for the selected dates`;
